...
var PrepareMailbody = /*#__PURE__*/function () {
  ...
  _createClass(PrepareMailbody, [{
    ...
    value: function build_forms(node_id) {
      ...
      // 実行ボタン
      var btn_run = utils_1.Utils.ce('input', 'btn btn-primary mb-3', [], '', {
        type: 'button',
        value: 'prepare',
      ...
      // 文字数上限入力フォーム
      var field_limit = utils_1.Utils.ce("input", "float-end ms-2", [], "", {
        id: constants_1.CONST.ID_LIMIT_LENGTH,
        value: "".concat(constants_1.CONST.LIMIT_LENGTH)
      });
      // FROMフィールド
      var textfield_from = PrepareMailbody.create_textarea(constants_1.CONST.ID_TEXT_FROM, "入力テキスト");
      textfield_from.addEventListener('change', PrepareMailbody.change_from);
      // TOフィールド
      var textfield_to = PrepareMailbody.create_textarea(constants_1.CONST.ID_TEXT_TO, "読み上げ用テキスト");
      textfield_to.addEventListener('change', PrepareMailbody.change_to);
      // クリップボードにコピーボタン
      var copy_to_cb = PrepareMailbody.create_copybutton(constants_1.CONST.ID_BTN_COPY, "copy to clipboard");
      // 全体を構築
      var formset = utils_1.Utils.ce('div', '', [utils_1.Utils.ce('span', 'float-end', [], '', {
        id: constants_1.CONST.ID_LENGTH_FROM
      }), textfield_from, utils_1.Utils.ce('div', 'float-end', [field_limit], "分割文字数の目安: "), utils_1.Utils.ce('div', '', [btn_run]), utils_1.Utils.ce('span', 'float-end', [], '', {
        id: constants_1.CONST.ID_LENGTH_TO
      }), textfield_to, copy_to_cb, utils_1.Utils.ce('div', '', [], '', {
    ...
    key: "create_textarea",
    value: function create_textarea(id, label) {
      //     <div class="mb-3">
      ...
      return utils_1.Utils.ce('div', 'mb-3', [utils_1.Utils.ce('label', 'form-label', [], label, {
        'for': id
      }), area]);
    }
    // クリップボードにコピーボタン
    ...
    value: function create_copybutton(id, label) {
      ...
      var copybutton = utils_1.Utils.ce('input', 'btn float-end mee-2', [], '', {
        id: constants_1.CONST.ID_BTN_COPY,
        type: "button",
        value: constants_1.CONST.VALUE_BTN_COPY
      });
