 ...
 */ class TutorialWidget {
   ...
   */ createModal() {
      ...
      <div class="tutorial-widget-modal">
        <button class="tutorial-widget-close" aria-label="\u{9589}\u{3058}\u{308B}">\xd7</button>
        ...
        <div class="tutorial-widget-footer">
          <label class="tutorial-widget-checkbox">
            ...
            <span>\u{6B21}\u{56DE}\u{306F}\u{8868}\u{793A}\u{3057}\u{306A}\u{3044}</span>
          </label>
          <div class="tutorial-widget-buttons">
            <button class="tutorial-widget-nav-btn" id="tutorial-widget-next">\u{3064}\u{304E}\u{3078}</button>
          </div>
        </div>
      </div>
    `;
        ...
        checkbox.addEventListener("change", (e)=>{
            if (e.target.checked) localStorage.setItem(this.storageKey, "true");
   ...
   */ async showPage(pageIndex) {
        ...
        if (page.highlightId) {
            ...
            if (this.container && !this.container.classList.contains("tutorial-widget-tooltip")) {
                ...
                this.createTooltip();
            } else if (!this.container) this.createTooltip();
            // ハイライトを作成
            ...
            if (this.container && this.container.classList.contains("tutorial-widget-tooltip")) {
                this.container.remove();
                this.createModal();
            } else if (!this.container) this.createModal();
        }
        // HTMLコンテンツを読み込んで表示
        try {
        ...
        // ナビゲーションボタンのテキストを更新
        const navBtn = this.container.querySelector("#tutorial-widget-next");
        if (pageIndex === this.pages.length - 1) navBtn.textContent = "\u304A\u308F\u308A";
        else navBtn.textContent = "\u3064\u304E\u3078";
    }
    /**
   * 次のページに進む（最後のページの場合は閉じる）
   */ nextPage() {
        if (this.currentPage < this.pages.length - 1) this.showPage(this.currentPage + 1);
        else this.close();
    }
    /**
   * モーダルを閉じる
   ...
   */ positionTooltip(targetElement) {
        ...
        else if (top + tooltipRect.height > viewportHeight - padding) top = viewportHeight - tooltipRect.height - padding;
        // 矢印のクラスを更新
